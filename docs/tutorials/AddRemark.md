...
This command allows users of the AddressBook application to add optional remarks to people in their taskDescription book and edit it if required. The command should have the following format:
...
Let’s start by creating a new `RemarkCommand` class in the `src/main/java/seedu/taskDescription/logic/command` directory.
...
/**
 * Changes the remark of an existing person in the taskDescription book.
 */
...
Create a `RemarkCommandParser` class in the `seedu.tr4cker.logic.parser` package. The class must extend the `Parser` interface.
...
Create a new `Remark` in `seedu.tr4cker.model.task`. Since a `Remark` is a field that is similar to `Address`, we can reuse a significant bit of code.
...
Simply add the following to [`seedu.tr4cker.ui.TaskCard`](https://github.com/se-edu/addressbook-level3/commit/850b78879582f38accb05dd20c245963c65ea599#diff-0c6b6abcfac8c205e075294f25e851fe).
...
:exclamation: You must delete AddressBook’s storage file located at `/data/tr4cker.json` before running it! Not doing so will cause AddressBook to default to an empty taskDescription book!
...
//...
    ...
    public CommandResult execute(Model model) throws CommandException {
        ...
        Person editedPerson = new Person(personToEdit.getName(), personToEdit.getDeadline(), personToEdit.getCompletionStatus(),
