...
In this tutorial, we’ll do exactly just that and remove the `taskDescription` field from `Person`.
...
1. Safe delete the field `taskDescription` in `EditPersonDescriptor`.
...
1. Remove the usages of `taskDescription` and select `Do refactor` when you are done.

   <div markdown="span" class="alert alert-primary">

   :bulb: **Tip:** Removing usages may result in errors. Exercise discretion and fix them. For example, removing the `taskDescription` field from the `Person` class will require you to modify its constructor.
   </div>
...
Unfortunately, there are usages of `Address` that IntelliJ IDEA cannot identify. You can find them by searching for instances of the word `taskDescription` in your code (`Edit` \> `Find` \> `Find in path`).

Places of interest to look out for would be resources used by the application. `main/resources` contains images and `fxml` files used by the application and `test/resources` contains test data. For example, there is a `$taskDescription` in each `PersonCard` that has not been removed nor identified.

![$taskDescription](../images/remove/$taskDescription.png)
...
<Label fx:id="taskDescription" styleClass="cell_small_label" text="\$taskDescription" />
...
In `src/test/data/`, data meant for testing purposes are stored. While keeping the `taskDescription` field in the json files does not cause the tests to fail, it is not good practice to let cruft from old features accumulate.
...
You can go through each individual `json` file and manually remove the `taskDescription` field.
