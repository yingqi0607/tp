...
public class AddCommandParserTest {
    ...
    public void parse_allFieldsPresent_success() {
        ...
        assertParseSuccess(parser, PREAMBLE_WHITESPACE + NAME_DESC_2 + DEADLINE_DESC_2
                + ADDRESS_DESC_2 + TAG_DESC_FRIEND, new AddCommand(expectedTask));
        ...
        assertParseSuccess(parser, NAME_DESC_2 + DEADLINE_DESC_2 + ADDRESS_DESC_1
                + ADDRESS_DESC_2 + TAG_DESC_FRIEND, new AddCommand(expectedTask));

        // multiple tags - all accepted
        Task expectedTaskMultipleTags = new TaskBuilder(MANUALTASK2).withTags(VALID_TAG_FRIEND, VALID_TAG_HUSBAND)
                .build();
        assertParseSuccess(parser, NAME_DESC_2 + DEADLINE_DESC_2 + ADDRESS_DESC_2
    ...
    public void parse_optionalFieldsMissing_success() {
        ...
        assertParseSuccess(parser, NAME_DESC_1 + DEADLINE_DESC_1 + ADDRESS_DESC_1,
    ...
    public void parse_compulsoryFieldMissing_failure() {
        ...
        // missing name prefix
        assertParseFailure(parser, VALID_NAME_2 + DEADLINE_DESC_2 + ADDRESS_DESC_2,
                expectedMessage);

        // missing deadline prefix
        assertParseFailure(parser, NAME_DESC_2 + VALID_DEADLINE_2 + ADDRESS_DESC_2,
                expectedMessage);

        // missing address prefix
        assertParseFailure(parser, NAME_DESC_2 + DEADLINE_DESC_2 + VALID_ADDRESS_2,
                expectedMessage);

        // all prefixes missing
        assertParseFailure(parser, VALID_NAME_2 + VALID_DEADLINE_2 + VALID_ADDRESS_2,
                expectedMessage);
    }
    ...
    public void parse_invalidValue_failure() {
        // invalid name
        assertParseFailure(parser, INVALID_NAME_DESC + DEADLINE_DESC_2 + ADDRESS_DESC_2
                + TAG_DESC_HUSBAND + TAG_DESC_FRIEND, Name.MESSAGE_CONSTRAINTS);

        // invalid deadline
        assertParseFailure(parser, NAME_DESC_2 + INVALID_DEADLINE_DESC + ADDRESS_DESC_2
                + TAG_DESC_HUSBAND + TAG_DESC_FRIEND, Deadline.MESSAGE_CONSTRAINTS);

        // invalid address
        assertParseFailure(parser, NAME_DESC_2 + DEADLINE_DESC_2 + INVALID_ADDRESS_DESC
                + TAG_DESC_HUSBAND + TAG_DESC_FRIEND, Address.MESSAGE_CONSTRAINTS);

        // invalid tag
        assertParseFailure(parser, NAME_DESC_2 + DEADLINE_DESC_2 + ADDRESS_DESC_2
                + INVALID_TAG_DESC + VALID_TAG_FRIEND, Tag.MESSAGE_CONSTRAINTS);

        // two invalid values, only first invalid value reported
        assertParseFailure(parser, INVALID_NAME_DESC + DEADLINE_DESC_2 + INVALID_ADDRESS_DESC,
